...
//object AllParamLens extends SampleLens[Sample] {
...
//        val dsigma =
//            -n / sigma + 1.0 / sigma3 * data.map(x => (x - mu) * (x - mu)).sum
//
...
//        val gradMu =
//            -(sample.parameters.mu - priorDistMu.mean) / priorDistMu.variance
//        val gradSigma =
//            -(sample.parameters.sigma - priorDistSigma.mean) / priorDistSigma.variance
//        sample.copy(
...
//            .withGradient[Sample](PriorEvaluator, LikelihoodEvaluator(data))
//
